...
namespace Client
{
    public class Client : MarshalByRefObject, IClient
    {
        ...
        private int splitFile(int splits)
        {
            ...
            // Creates a buffer to hold the split and the extra bytes (until the next '\n')
            ...
            using (var input = new FileStream(this.inputPath, FileMode.Open, FileAccess.Read))
            {
                while (input.Position < input.Length)
                {
                    ...
                    splitinfo.pos = fileIndex;
                    splitinfo.length = buffer.Length + extraBufferArray.Length;
